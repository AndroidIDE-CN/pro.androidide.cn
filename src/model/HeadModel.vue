...
<template>
    <div class="head-container" :style="{ height: functionStore.isMobileHeadMore ? `${75 + 45 * (siteConfig.linkList.length + 2)}px` : applicationStore.isDeviceMobile ? '65px' : '75px' }" v-if="readyStatus">
        ...
        <div class="head-more" :style="{ height: functionStore.isMobileHeadMore ? `${45 * (siteConfig.linkList.length + 2)}px` : '0px', paddingBottom: functionStore.isMobileHeadMore ? '10px' : '0px' }">
            <div class="link-item">
                <span class="link-text" @click="functionStore.setIsUpdateOpen(true)">更新日志</span>
            </div>
            <div class="link-item">
