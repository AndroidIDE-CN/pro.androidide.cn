...
<script setup>
    ...
    onMounted(async () => {
        await utils.axiostool.sendHttpGet(utils.apiConfig.main).then((data) => {
            readyStatus.value = true;
            siteConfig.value = data.data;

            try {
                siteConfig.value.preview.forEach(element => {
...
<template>
    <div class="main-container" :style="{ minHeight: applicationStore.isDeviceMobile ? 'calc(100vh - 65px)' : '600px', background: applicationStore.isDeviceMobile ? '#ffffff' : 'rgb(248 250 252)' }" :class="applicationStore.isDeviceMobile && 'container-mobile'" v-if="readyStatus">
            ...
            <div class="container">
                <div class="left-introduce">
                    ...
                    <div class="button-group">
                        <el-dropdown :style="{ lineHeight: 'unset',verticalAlign: 'unset',height: 'auto',fontSize: 'unset' }" v-if="siteConfig.downloadHref.length > 1">
                            <a class="button-link" :style="{ background: '#495057', color: '#ffffff', border: '1px solid #495057' }" target="_blank" title="立即下载" :href="siteConfig.downloadHref[0].href">立即下载</a>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item v-for="(item,index) in siteConfig.downloadHref" :key="index" @click="letDownload(item.href)">{{ item.name }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
                        <a class="button-link" :style="{ background: '#495057', color: '#ffffff', border: '1px solid #495057' }" target="_blank" title="立即下载" :href="siteConfig.downloadHref[0].href" v-else>立即下载</a>
                ...
                <img class="screenhot" v-lazy="preview" draggable="false" :alt="siteConfig.title">
            </div>
        ...
        <div class="main-box box-mobile" v-if="applicationStore.isDeviceMobile">
            <div class="container">
                ...
                <div class="left-introduce">
                    ...
                    <div class="button-group">
                        <span class="button-link" :style="{ background: '#495057', color: '#ffffff', border: '1px solid #495057' }" @click="showBottom = true" v-if="siteConfig.downloadHref.length > 1">立即下载</span>
                        <a class="button-link" :style="{ background: '#495057', color: '#ffffff', border: '1px solid #495057' }" target="_blank" title="立即下载" :href="siteConfig.downloadHref[0].href" v-else>立即下载</a>
                        <span class="button-link" :style="{ background: '#ffffff', color: '#495057', border: '1px solid #495057' }" @click="functionStore.setIsHisVerOpen(true)">历史版本</span>
                    </div>
        ...
        <van-action-sheet v-model:show="showBottom" :actions="siteConfig.downloadHref" cancel-text="取消" close-on-click-action @select="onDownSelect" description="请选择下载路线"/>
    </div>
</template>
