...
<script setup>
    ...
    const getTableData = async () => {
        await axiostool.sendGetRequest(`admin/download/query?page=${pages.value}&limit=${limit.value}`).then(async (response) => {
    ...
    const deleteItem = async (id) => {
        await axiostool.sendPostRequest("admin/download/delete", { id: id }).then(async (response) => {
            if (response.code !== 200) {
                ElMessage({ message: response.message, type: 'error', plain: true});
            } else {
    ...
    const sendEditRequest = async () => {
        if (formModel.name === "" || formModel.url === "" || formModel.description === "") return ElMessage({ message: "请填写完整信息", type: 'warning', plain: true});
        await axiostool.sendPostRequest(`admin/download/${ editModel.value ? 'add' : 'update' }`, formModel).then(async (response) => {
            if (response.code !== 200) {
                ElMessage({ message: response.message, type: 'error', plain: true});
            } else {
                ElMessage({ message: response.message, type: 'success', plain: true});
...
<template>
    <div class="content-frame frame-friends">
        <div class="action-box">
            <el-button type="primary" size="large" :icon="Plus" @click="addItem">新增路线</el-button>
        ...
        <el-table :data="tableData" border style="width: 100%; margin: 20px 0;">
            ...
            <el-table-column prop="url" label="路线链接"/>
        ...
        <el-dialog v-model="dialogStatus" :title="`${editModel ? '新增' : '修改'}路线`" width="520" align-center>
            <el-form :model="formModel" label-width="auto">
                <el-form-item label="路线名称">
                    <el-input v-model="formModel.name" placeholder="请输入路线名称" clearable/>
                </el-form-item>

                <el-form-item label="路线链接">
                    <el-input v-model="formModel.url" placeholder="请输入路线链接" clearable/>
                </el-form-item>

                <el-form-item label="路线描述">
                    <el-input type="textarea" v-model="formModel.description" placeholder="请输入路线描述"/>
                </el-form-item>
            </el-form>
